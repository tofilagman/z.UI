using z.UI.TabControl.TabStyleProviders;
using System;
    ...
    public abstract class TabStyleProvider : Component
    {
        ...
        public Point Padding
        {
            ...
            set
            {
                ...
                {
                    if (value.X + Convert.ToInt32(this._Radius / 2) < -6)
                    {
                        ((System.Windows.Forms. TabControl)this._TabControl).Padding = new Point(0, value.Y);
                    }
                    else
                    {
                        ((System.Windows.Forms.TabControl)this._TabControl).Padding = new Point(value.X + Convert.ToInt32(this._Radius / 2) + 6, value.Y);
                    }
                }
                else
                {
                    if (value.X + Convert.ToInt32(this._Radius / 2) < 1)
                    {
                        ((System.Windows.Forms.TabControl)this._TabControl).Padding = new Point(0, value.Y);
                    }
                    else
                    {
                        ((System.Windows.Forms.TabControl)this._TabControl).Padding = new Point(value.X + Convert.ToInt32(this._Radius / 2) - 1, value.Y);
                    }
                }
            }
        }
        ...
        private void DrawTabFocusIndicator(GraphicsPath tabpath, int index, Graphics graphics)
        {
            if (this._FocusTrack && this._TabControl.Focused && index == this._TabControl.SelectedIndex)
            {
                ...
                {
                    case TabAlignment.Top:
                        focusRect = new Rectangle(Convert.ToInt32(Math.Truncate(pathRect.X)), Convert.ToInt32(Math.Truncate(pathRect.Y)), Convert.ToInt32(Math.Truncate(pathRect.Width)), 4);
                        focusBrush = new LinearGradientBrush(focusRect, this._FocusColor, SystemColors.Window, LinearGradientMode.Vertical);
                        break; // TODO: might not be correct. Was : Exit Select

                        
                    case TabAlignment.Bottom:
                        focusRect = new Rectangle(Convert.ToInt32(Math.Truncate(pathRect.X)), Convert.ToInt32(Math.Truncate(pathRect.Bottom)) - 4, Convert.ToInt32(Math.Truncate(pathRect.Width)), 4);
                        focusBrush = new LinearGradientBrush(focusRect, SystemColors.ControlLight, this._FocusColor, LinearGradientMode.Vertical);
                        break; // TODO: might not be correct. Was : Exit Select

                        
                    case TabAlignment.Left:
                        focusRect = new Rectangle(Convert.ToInt32(Math.Truncate(pathRect.X)), Convert.ToInt32(Math.Truncate(pathRect.Y)), 4, Convert.ToInt32(Math.Truncate(pathRect.Height)));
                        focusBrush = new LinearGradientBrush(focusRect, this._FocusColor, SystemColors.ControlLight, LinearGradientMode.Horizontal);
                        break; // TODO: might not be correct. Was : Exit Select
 
                        
                    case TabAlignment.Right:
                        focusRect = new Rectangle(Convert.ToInt32(Math.Truncate(pathRect.Right)) - 4, Convert.ToInt32(Math.Truncate(pathRect.Y)), 4, Convert.ToInt32(Math.Truncate(pathRect.Height)));
        ...
        {
            float[] relativeIntensities = new float[] {
            0f,
            0.7f,
            1f
        };
            ...
            {
                relativeIntensities = new float[] {
                0f,
                0.5f,
                1f,
                1f
            };
                relativePositions = new float[] {
                0f,
                0.5f,
                0.51f,
                1f
